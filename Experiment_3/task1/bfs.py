...
def bfs(start_x, start_y, end_x, end_y, queue_x, queue_y, data, visited, father):   # 广度优先
    ...
    for i in range(4):  # 针对上下左右四个不同方向
        ...
        if data[x][y] != '1' and visited[x][y] == 1:  # 可被访问
            visited[x][y] = 0
            father[x][y] = [start_x, start_y]  # 记录当前点的来源
            if visited[end_x][end_y] == 0:  # 如果到达了终点
                return
            queue_x.push(x)  # 入队列
            queue_y.push(y)
    if queue_x.empty():  # 如果队列为空
        return
    x = queue_x.front()  # 获取队列中第一个点，并做出队列操作
    y = queue_y.front()
    queue_x.dequeue()
    queue_y.dequeue()
    bfs(x, y, end_x, end_y, queue_x, queue_y, data, visited, father)
    return
...
def main():
    ...
    queue_x, queue_y = Queue(), Queue()  # 用于记录未访问的可达点的坐标
    bfs(start[0], start[1], end[0], end[1], queue_x, queue_y, list_of_data, visited, father)
